...
import {
    Clock, AlertCircle, PlayCircle, Info, X,
    Zap, GitBranch, ExternalLink, MessageSquare,
    Search, Loader2, ListChecks, RefreshCw,
    Copy, GitPullRequest
} from 'lucide-react';
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100/50">
                            ...
                            <div className="flex items-center gap-2">
                                <span className={`w-1.5 h-1.5 rounded-full ${task.status === 'done' ? 'bg-emerald-500' : 'bg-indigo-500 animate-pulse'}`} />
                            ...
                            <div className="bg-indigo-50/30 rounded-2xl p-4 border border-indigo-100/50">
                                <div className="space-y-3 mb-4">
                                    <div className="flex items-center justify-between bg-white p-2 rounded-xl border border-gray-100 shadow-sm">
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Branch Name</span>
                                                <code className="text-xs font-mono font-bold text-indigo-900 truncate max-w-[200px] sm:max-w-xs">{task.github_branch}</code>
                                            </div>
                                        </div>
                                        <button
                                            onClick={() => copyToClipboard(task.github_branch)}
                                            className="p-1.5 hover:bg-indigo-50 rounded-lg transition-all text-gray-400 hover:text-indigo-600 shrink-0"
                                    ...
                                    <div className="flex items-center justify-between bg-white p-2 rounded-xl border border-gray-100 shadow-sm">
                                        <div className="flex items-center gap-2 overflow-hidden">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Full GitHub URL</span>
                                                <code className="text-xs font-mono font-bold text-blue-600 truncate max-w-[200px] sm:max-w-xs">{task.github_branch_url || 'Generating...'}</code>
                                            </div>
                                        </div>
                                        <button
                                            onClick={() => copyToClipboard(task.github_branch_url)}
                                            className="p-1.5 hover:bg-blue-50 rounded-lg transition-all text-gray-400 hover:text-blue-600 shrink-0"
                                            title="Copy full URL"
                                            disabled={!task.github_branch_url}
                                        >
                                            <Copy size={14} />
                                        </button>
                                    </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <a
                                        ...
                                        className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-xl text-[11px] font-black text-gray-700 hover:bg-gray-50 hover:border-indigo-200 transition-all shadow-sm flex-1 justify-center"
                                    >
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center gap-2 px-4 py-2 bg-indigo-600 rounded-xl text-[11px] font-black text-white hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 flex-1 justify-center"
                                    >
    ...
    return (
        <div className="space-y-6">
            ...
            <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 font-black">My Assigned Tasks</h1>
                    <p className="text-gray-500 mt-1 uppercase text-[11px] font-bold tracking-widest flex items-center gap-2">
                        ...
                        Actionable work items from Jira
                    </p>
                </div>
                <div className="flex items-center gap-2">
